...
import {randomElOrder, 
  setElHeights, 
  makeArrayElsVisible
} from './js/setup';
import {elValue} from './js/sorting_utils';
import {addPhase1Text,
  addPhase2Text,
  addPhase3Text,
  addPhase4Text,
  setText,
...
const _addToDOMArray = (id, element) => {
  ...
  if (id === 'left') {
    moveByAnimation(arr, [-1, 0, 0]);
  } else {
    moveByAnimation(arr, [1, 0, 0]);
  }
  const xOffset = sortingContainer.numChildren; // need to check
